...
public interface MedicoRepositorio extends JpaRepository<Medico, Long> {
	...
	@Query("SELECT m " + "FROM Medico m " + "LEFT JOIN m.usuarios u " + "GROUP BY m.id, m.nombre "
	...
	@Query("SELECT m " + "FROM Medico m " + "LEFT JOIN m.usuarios u " + "WHERE m.id <> :medicoId "
			+ "GROUP BY m.id, m.nombre " + "ORDER BY COUNT(u.id) ASC " + "LIMIT 1")
